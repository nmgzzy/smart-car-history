...
void data_read(uint8 n)
{
    ...
    {
        temp=flash_read(255-n,2*i,uint16);
        CanShu[i]=temp;
    }
    //把flash的参数赋给变量
    pid_angle.p = CanShu[0] / 100.0f;
    pid_angle.d = CanShu[1] / 100.0f;
    pid_speed.p = CanShu[2] / 100.0f;
    pid_speed.i = CanShu[3] / 100.0f;
    pid_speed.d = CanShu[4] / 100.0f;
    pid_dir_pset= CanShu[5] / 100.0f;
    pid_dir.d   = CanShu[6] / 100.0f;
...
{
    //把变量的数据赋给flash数组
    CanShu[0] = (int16)(pid_angle.p * 100.0f);
    CanShu[1] = (int16)(pid_angle.d * 100.0f);
    CanShu[2] = (int16)(pid_speed.p * 100.0f);
    CanShu[3] = (int16)(pid_speed.i * 100.0f);
    CanShu[4] = (int16)(pid_speed.d * 100.0f);
    CanShu[5] = (int16)(pid_dir_pset * 100.0f);
    CanShu[6] = (int16)(pid_dir.d * 100.0f);
    ...
    FLASH_EraseSector(255-n);
    FLASH_WriteSector(255-n,(const uint8 *)CanShu,NUM_OF_CANSHU*2,0);
}
