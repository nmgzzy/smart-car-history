...
def display(page, x, y, k, save):
    ...
    if save:
        ...
        img_cnt += 1
    global colorxy
    blobs = img.find_blobs([red_threshold], roi = (47,25,103,70), x_stride=5, y_stride=10, pixels_threshold=200)
    if blobs:
        obstacle = find_max(blobs)
        img.draw_rectangle(obstacle.rect())

    blobs = img.find_blobs([blue_threshold, black_threshold], roi = roiblue[0], x_stride=3, y_stride=6, pixels_threshold=100)
    if blobs:
        road = find_max(blobs)
        img.draw_rectangle(road.rect())
    blobs = img.find_blobs([blue_threshold, black_threshold], roi = roiblue[2], x_stride=3, y_stride=6, pixels_threshold=36)
    if blobs:
        road = find_max(blobs)
        img.draw_rectangle(road.rect())
...
def main():
    while True:
        ...
        if t!= None:
            mode = t >> 7
            offset = t & 0x7F
        blobs = img.find_blobs([red_threshold], roi = (47,25,87,70), x_stride=5, y_stride=10, pixels_threshold=200)
        if blobs:
            obstacle = find_max(blobs)
        ...
        if mode == 0:
            ...
            blobs = img.find_blobs([blue_threshold, black_threshold], roi = roiblue[1], x_stride=3, y_stride=6, pixels_threshold=64)
            if blobs:
                road = find_max(blobs)
                cnt1 = road.pixels()
                #img.draw_rectangle(road.rect())
