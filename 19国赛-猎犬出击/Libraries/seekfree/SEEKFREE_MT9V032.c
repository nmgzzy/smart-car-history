...
uint8   image[ROW][COL];      //图像原始数组//public
uint8   receive[3];
uint8   receive_num = 0;
...
{
    {AUTO_EXP,          32},   //自动曝光设置      范围1-63 0为关闭 如果自动曝光开启  EXP_TIME命令设置的数据将会变为最大曝光时间，也就是自动曝光时间的上限
...
//  @return     void
//  @since      v1.0
//  Sample usage:				调用该函数前请先初始化DEBUG_UART
//-------------------------------------------------------------------------------------------------------------------
...
{
	uart_putchar(DEBUG_UART,0x00);uart_putchar(DEBUG_UART,0xff);uart_putchar(DEBUG_UART,0x01);uart_putchar(DEBUG_UART,0x01);//发送命令
    uart_putbuff(DEBUG_UART, (uint8_t *)image, ROW*COL);  //发送图像
}