...
void showimage(const unsigned char *p)
{
	...
	dsp_single_colour(TFT_WHITE); //清屏
...
void lcd_displayimage_bin(uint8 *p, uint16 width, uint16 height, uint8 x, uint8 y)
{
    ...
    uint16 color[9]={TFT_BLACK,TFT_WHITE,TFT_RED,TFT_GREEN,TFT_BLUE,
                        TFT_CYAN,TFT_PINK,TFT_YELLOW,TFT_ORANGE};
	uint16 temp = 0;
    ...
    coord_x = width>TFT_X_MAX?TFT_X_MAX:width;
    coord_y = height>TFT_Y_MAX?TFT_Y_MAX:height;
    lcd_setregion(x,y,x+coord_x-1,y+coord_y-1);
    ...
    for(j=0;j<coord_y;j++)
    {
        ...
        {
            temp = *(p+j*width+i*width/coord_x);//读取像素点
            lcd_writedata_16bit(color[temp]);
        }
    }
