...
void PIT0_IRQHandler(void)
{
    ...
    UINT bb;
    BYTE img[8*128];
    sprintf(buf,"0:/img%d.dat",time_count);
    if(!f_open(&fp, buf, FA_READ))
    {
        f_read (&fp, img, 8*128, &bb);
        f_close(&fp);
        dis_bmp_bin(img);
    }
    time_count++;
...
void UART2_RX_TX_IRQHandler(void)
{
    if(UART2->S1 & UART_S1_RDRF_MASK)                                     //接收数据寄存器满
    {
        ...
        if(com_receive_flag == 1 && time_count > 0)
        {
            ...
            if(com_format == ANG_)
            {
                img_offset = com_receive_data[0];
            }
            if(com_format == BLC_)
            {
                Balance_mode = com_receive_data[0];
            }
            if(com_format == DAT_)
            {
                if(flag.SD == SD_BIN)
                {
                    for(uint8 i=0; i<24; i++)
                        image_save[2048+i] = com_receive_data[i];
                }
                else
                {
                    for(uint8 i=0; i<24; i++)
                        image_save[16384+i] = com_receive_data[i];
                }
            }
        }
    }
