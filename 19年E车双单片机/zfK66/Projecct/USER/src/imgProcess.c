...
void image_deal(void)
{
    if(flag.SD == SD_GRY)
    {
        memcpy(image_save, image[0+img_offset], 16200);
    }
    resize_image(image[0+img_offset], image_s[0]);
    Threshold = otsu_threshold(image_s[0], Threshold) + Threshold_offset;
    image_binaryzation(image[0+img_offset], image_bin[0], Threshold);
    if(flag.SD == SD_BIN)
    {
        uint16 i;
...
{
    uint16 i,j,t;
    int8 Coffset = 0, Roffset = 0;
    for(i = 0; i < ROW; i++)
    {
        t = i * COL;
        Roffset = (int8)(0.1111f*i-5);
        for(j = 0; j < COL; j++)
        {
            Coffset = (int8)(-0.002822f*j*j+0.5079f*j-20);
            if(*(image_src+t+j) > Threshold + Roffset + Coffset)
                *(image_bin+t+j) = 1;
            else
                *(image_bin+t+j) = 0;
        }
