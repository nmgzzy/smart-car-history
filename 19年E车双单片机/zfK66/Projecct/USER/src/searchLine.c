...
uint8 flag_broken_road2 = 0;
uint8 flag_broken_road_last = 0;
uint8 flag_broken_road_last2 = 0;
...
void SearchLines(void)
{
    ...
    if (valid_line > ROW / 3)//检测短路
    {
        if(DetectBrokenRoad() || valid_line >= ROW - 5)
            flag_broken_road = 1;
        else
            flag_broken_road = 0;
    }
        ...
        flag_broken_road = 0;
    if(flag_broken_road != flag_broken_road_last)
    {
        communicate_send(&flag_broken_road, ROD_, 1);
        break_cnt = 0;
    }
    else
    {
    ...
    {
        break_cnt = 20;
        flag_broken_road2 = flag_broken_road;
    }
    if(flag_broken_road2 == 1 && flag_broken_road_last2 == 0)
        communicate_send(&flag_broken_road2, SWC_, 1);
    flag_broken_road_last = flag_broken_road;
    flag_broken_road_last2 = flag_broken_road2;


    /*if(障碍)
    {
        is_red = 0;
        for(Ysite = 0; Ysite < 20; Ysite+=2)
        {
            for(Xsite = imgMid - 15; Xsite < imgMid + 15; Xsite+=3)
            {
                is_red += image[Ysite][Xsite];
            }
        }
    }*/
