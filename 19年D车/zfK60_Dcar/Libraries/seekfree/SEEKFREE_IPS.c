...
//-----------------引脚定义------------------------------
#define IPS_SPI         spi0
#define IPS_SPI_CS      SPI_PCS1
#define IPS_CS_PIN      C18
#define IPS_BL_PIN      D7              //液晶背光引脚定义
#define IPS_REST_PIN    D0              //液晶复位引脚定义
#define IPS_DC_PIN      C19             //液晶命令位引脚定义
...
{
    IPS_DC(0);
    spi_mosi(IPS_SPI,IPS_SPI_CS,&dat,&dat,1);
}
...
void ips_writedata_16bit(uint16 dat)	//向液晶屏写一个16位数据
{
    ...
    IPS_DC(1);
    spi_mosi(IPS_SPI,IPS_SPI_CS,&h,&h,1); 	//写入高8位数据
    spi_mosi(IPS_SPI,IPS_SPI_CS,&l,&l,1);	//写入低8位数据
}
...
void ips_init(void)
{
    spi_init(IPS_SPI, IPS_SPI_CS, MASTER, 25*1000*1000);//硬件SPI初始化

    gpio_init(IPS_CS_PIN,GPO,0);
...
void ips_show_str(uint16 x,uint16 y,uint8 dat[])
{
	...
	while(dat[j] != '\0')
	{
		ips_show_char(x+8*j,y*16,dat[j]);
		j++;
	}
}
...
//  @return     void
//  @since      v1.0
//  Sample usage:               ips_showbuff(0,0,"seekfree",20);
//-------------------------------------------------------------------------------------------------------------------
void ips_show_buff(uint16 x,uint16 y,uint8 dat[], uint8 n)
{
	uint16 j;
    if(x+8*n > IPS_X_MAX) n = (IPS_X_MAX - x)/8;
	j = 0;
	while(j < n)
	{
		ips_show_char(x+8*j,y*16,dat[j]);
...
void ips_show_int8(uint16 x,uint16 y,int8 dat)
{
	...
	if(dat<0)
	{
		ips_show_char(x,y*16,'-');
		dat = -dat;
	}
	else	ips_show_char(x,y*16,' ');
	...
	while(i<3)
	{
		ips_show_char(x+(8*(i+1)),y*16,'0' + a[i]);
		i++;
	}
}
...
void ips_show_uint8(uint16 x,uint16 y,uint8 dat)
{
	...
	while(i<3)
	{
		ips_show_char(x+(8*i),y*16,'0' + a[i]);
		i++;
	}
...
void ips_show_float(uint16 x,uint16 y,double dat,uint8 num,uint8 pointnum)
{
    ...
    ips_show_str(x, y, (uint8 *)buff);	//显示数字
}
