...
const uint8 F6x8[][6] =
{
	...
	{ 0x00, 0x3E, 0x41, 0x49, 0x41, 0x3E },   // 0
...
void OLED_fill(uint8 x1,uint8 x2,uint8 y,uint8 bmp_data)
{
    int j;
    OLED_WrCmd(0xb0+y);
    OLED_WrCmd(((x1&0xf0)>>4)|0x10);
    OLED_WrCmd((x1&0x0f)|0x01);
    for(j=x1;j<x2+1;j++)	OLED_WrDat(bmp_data);
}
...
void OLED_HEXACSII1(uint16 hex,uint8* Print)
{
	...
	while(i>j)
	{
	  	t = Print[i]; Print[i--] = Print[j]; Print[j++] = t;
	}
...
//  @param      num         无符号数
//  @return     void
//  @since      v1.0	v2.0 by NNU ZZY
//  Sample usage:
//-------------------------------------------------------------------------------------------------------------------
void OLED_Print_uint16(uint8 x, uint8 y, uint16 num, uint8 have0, uint8 font)
{
	uint8 *ch1,ch[6];
	if(have0 == 0)OLED_HEXACSII1(num,ch);
    else OLED_HEXACSII(num,ch);
	ch1 =&ch[0];
	while(*ch1 != '\0')
	{
		if(font == 0)
